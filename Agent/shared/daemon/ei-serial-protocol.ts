...
export default class EiSerialProtocol {
    ...
    async getConfig() {
        ...
        let section: 'info' | 'sensors' | 'wifi' | 'inference' | 'sampling' | 'upload' | 'management' | 'snapshot' | undefined;
        ...
        for (let line of data.split('\n').map(l => l.trim()).filter(l => !!l)) {
            ...
            if (section === 'info') {
                ...
                if (key === 'at version') {
                    ...
                    config.info.atCommandVersion = { major, minor, patch };
                    continue;
                }
    ...
    async takeSnapshot(width: number, height: number): Promise<EiSnapshotResponse> {
        ...
        (async () => {
            ...
            try {
                ...
                // const img = Buffer.from(last, 'base64');
                // require('fs').writeFileSync("image.png", img);
     ...
     */
    private waitForSerialSequence(originalCommand: string, seq: Buffer, timeout: number, logProgress: boolean = false): Promise<Buffer> {
        let total = 0;
    ...
    private async execCommandAutoSwitchBaudRate(command: string,
        ...
        if (useMaxBaudRate && this._config.info.transferBaudRate) {
            ...
            await this.waitForSerialSequence(command, Buffer.from([ 0x0d, 0x0a, 0x4f, 0x4b ]), 30000, logProgress);
            ...
            let data1 = await this.waitForSerialSequence(command, Buffer.from([ 0x0d, 0x0a, 0x4f, 0x4b ]), timeout, logProgress);
    ...
    private shouldUseMaxBaudrate(command: string) {
        ...
        let hasFasterBaudrate = typeof this._config.info.transferBaudRate === 'number' && this._config.info.transferBaudRate !== 115200;
