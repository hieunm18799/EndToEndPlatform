...
export async function setupCliApp(configFactory: Config, config: EdgeImpulseConfig, opts: {
    ...
    hmacKeyArgv: string | undefined,
    connectProjectMsg: string,
    getProjectFromConfig?: (deviceId: string | undefined) 
    ...
    if (projectId) {
        try {
            ...
            if (!opts.silentArgv) {
                console.log('    Project:    ', projectInfoReq.name + ' (ID: ' + projectId + ')');
                console.log('');
            }
        }
        catch (ex2) {
    ...
    if (!projectId) {
        ...
        let projectList = (await config.api.projects.listProjects({headers: {'x-jwt-user': config.api.apiKey}}));
        ...
        if (!projectList || projectList.length === 0) {
            ...
            let inqRes = await inquirer.prompt([{
                type: 'list',
                choices: (projectList || []).map(p => ({ name: p.owner + ' / ' + p.name, value: p.id })),
                name: 'project',
                message: opts.connectProjectMsg,
                pageSize: 20
            }]);
            projectId = String(inqRes.project);
        }
    }
    ...
    if (!opts.apiKeyArgv) {
        try {
            let dk = (await config.api.projects.getProjectKey(projectId ? projectId : '', {headers: {'x-jwt-user': config.api.apiKey}}));
            console.log(dk);

            if (dk) devKeys.apiKey = dk;
