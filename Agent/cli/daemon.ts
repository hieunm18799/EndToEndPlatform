...
import { SERIAL_STATE } from '../shared/daemon/iserialconnector';
...
class SerialDevice extends (EventEmitter as new () => TypedEmitter<{
    ...
    private _deviceConfig: EiSerialDeviceConfig | undefined;
    ...
    constructor(config: EdgeImpulseConfig, serialConnector: SerialConnector, serialProtocol: EiSerialProtocol, deviceConfig?: EiSerialDeviceConfig) {
        super();
    ...
    async getDeviceId() {
        return this._deviceConfig? this._deviceConfig.info.id : "";
    }

    async serialDisconnect() {
        console.log('disconnect');
        return await this._serial.disconnect();
    }
    ...
    getDeviceType() {
        return this._deviceConfig ? this._deviceConfig.info.type : undefined;
    }

    getSensors() {
        if (!this._deviceConfig) return undefined;
    ...
    supportsSnapshotStreaming() {
        return this._deviceConfig ? this._deviceConfig.snapshot.supportsStreaming : false;
    }
    ...
    async startSnapshotStreaming() {
        ...
        this._snapshotStream.ee.on('snapshot', async (buffer, width, height) => {
            ...
            try {
                if (Date.now() - +this._lastSnapshot > 200 && id === this._snapshotId) {

                    let depth = buffer.length / (width * height);
                    if (depth !== 1 && depth !== 3) {
                        throw new Error('Invalid length for snapshot, expected ' + (width * height) + ' or ' + (width * height * 3) + ' values, but got ' + buffer.length);
                    }
    ...
    async beforeConnect() {
        ...
        if (this._deviceConfig.management.connected) {
            console.log(SERIAL_PREFIX, 'Device is connected over WiFi to remote management API, no need to run the daemon. Exiting...');
            process.exit(1);
        }
        else {
            if (!this._deviceConfig.upload.apiKey || !this._deviceConfig.info.id || !this._deviceConfig.info.type) {
                console.error(SERIAL_PREFIX, 'Cannot connect to remote management API using daemon, failed to read apiKey, deviceId or deviceType from device. Restarting your development board might help. Retrying in 5 seconds...');
                serial_connect;
                return;
            }

            console.log(SERIAL_PREFIX, 'Device is not connected to remote management API, will use daemon');
        }
    }
    ...
    async sampleRequest(data: MgmtInterfaceSampleRequestSample, ee: RemoteMgmtDeviceSampleEmitter) {
        ...
        if (!this._deviceConfig || !this._deviceConfig.upload.apiKey) {
        ...
        if (s.sensor?.startsWith('Camera (')) {
            ...
            if (depth !== 1 && depth !== 3) {
                throw new Error('Invalid length for snapshot, expected ' + (width * height) + ' or ' + (width * height * 3) + ' values, but got ' + snapshot.length);
            }

            console.log(depth, snapshot.length);
            // require("fs").writeFileSync("out.png", snapshot, {}, (err: Error) => {
            //     console.log(err);
            //   });
            ...
            console.log(snapshot[0], snapshot[1], snapshot[2], snapshot[3], snapshot[4], snapshot[5], snapshot[6], snapshot[7], snapshot[8]);
            ...
            let url = 'http://localhost:4810' + s.path;
            console.log(SERIAL_PREFIX, 'Uploading to', url);

            ee.emit('uploading');

            const sampleId = await upload({
                name: s.label + '.jpg',
                devKey: this._deviceConfig.upload.apiKey,
            ...
            await this._serialProtocol.setSampleSettings(s.label, s.interval, s.length, s.hmacKey);
            ...
            if (deviceResponse && deviceResponse.file) {
                ...
                try {
                    console.log(SERIAL_PREFIX, 'Uploading to', url);
                    if (deviceResponse.file && deviceResponse.file.indexOf(Buffer.from('Ref-BINARY-', 'ascii')) > -1) {
...
(async () => {
    try {
        ...
        // const config = await new Config().verifyLogin(devArgv, apiKeyArgv);
...
async function connectToSerial(eiConfig: EdgeImpulseConfig, deviceId: string, baudRate: number, clean: boolean) {
    ...
    const { projectId, devKeys } = await setupCliApp(configFactory, eiConfig, cliOptions, serialId);
    if (serialId) {
    ...
    if (!remoteMgmt) {
        ...
        // console.log(projectId, devKeys)
        remoteMgmt = new RemoteMgmt(projectId, devKeys, Object.assign({command: <'edge-impulse-daemon'>'edge-impulse-daemon'}, eiConfig), device,
            url => new WebSocket(url));
    
        const onSignal = async () => {
            console.log(SERIAL_PREFIX, 'Received stop signal, stopping application... Press CTRL+C again to force quit.');
            try {
    ...
    serial.on('error', async err => {
        console.log(SERIAL_PREFIX, 'Serial error - retrying conntect!', err);
        serial_connect;
    });
    serial.on('close', async () => {
        console.log(SERIAL_PREFIX, 'Serial closed - retrying conntect!');
        serial_connect;
    });
    async function connectLogic() {
        if (!serial || serial.isConnected()) await serial_connect;
        ...
        try {
            ...
            console.log(SERIAL_PREFIX, 'Device is running AT command version ' + config.info.atCommandVersion.major + '.' + config.info.atCommandVersion.minor + '.' + config.info.atCommandVersion.patch);
            ...
            if (config.info.atCommandVersion.major > 1 || config.info.atCommandVersion.minor > 8) {
                console.error(SERIAL_PREFIX, 'Unsupported AT command version running on this device. Supported version is 1.8.x and lower, but found ' + config.info.atCommandVersion.major + '.' + config.info.atCommandVersion.minor + '.' + config.info.atCommandVersion.patch + '.');
                console.error(SERIAL_PREFIX, 'Update the Edge Impulse CLI tools (via `npm update edge-impulse-cli -g`) to continue.');
                process.exit(1);
            }


            let setupRes = silentArgv ? { hasWifi: true, setupOK: true, didSetMgmt: false } : await setupWizard(eiConfig, serialProtocol, config, devKeys);
            if (setupRes.setupOK) {
...
async function serial_connect() {
    if (!serial) return;
    while (serial.getState() == SERIAL_STATE.DISCONNECT );
    ...
    try {
        ...
        let ex = <Error>ex2;
        console.error(SERIAL_PREFIX, 'Failed to connect to', serial.getPath(), 'retrying in 5 seconds', ex.message || ex);
...
async function setupWizard(eiConfig: EdgeImpulseConfig, 
    ...
    if (deviceConfig.wifi.connected
        && deviceConfig.management.connected
        && deviceConfig.upload.apiKey
        && deviceConfig.sampling.hmacKey
        && deviceConfig.management.url === `ws://localhost:4802`
        && deviceConfig.upload.host === `http://localhost:4810`) {
    ...
    try {
        ...
        if (deviceConfig.info.id === '00:00:00:00:00:00' && deviceConfig.info.atCommandVersion.major >= 1 && deviceConfig.info.atCommandVersion.minor >= 4 && serial) {
        ...
        if (deviceConfig.upload.host !== `http://localhost:4810`) {
            if (true) {
                process.stdout.write('Setting upload host in device...');
                await serialProtocol.setUploadHost(`http://localhost:4810`);
                process.stdout.write(' OK\n');
            }
        }

        if (true && deviceConfig.management.url !== `ws://localhost:4802`) {
            process.stdout.write('Configuring remote management settings...');
            await serialProtocol.setRemoteManagement(`ws://localhost:4802`);
            process.stdout.write(' OK\n');
            ret.didSetMgmt = true;
        }
        ...
        if ((!deviceConfig.sampling.hmacKey || deviceConfig.sampling.hmacKey === '') && devKeys.hmacKey) {
            process.stdout.write('Configuring HMAC key in device...');
            await serialProtocol.setSampleSettings(deviceConfig.sampling.label, deviceConfig.sampling.interval, deviceConfig.sampling.length, devKeys.hmacKey);
            process.stdout.write(' OK\n');
        }

        if (!deviceConfig.wifi.connected && credentials.askWifi !== false && deviceConfig.wifi.present && !setupWizardRan) {
